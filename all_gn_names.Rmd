...
```
This code compiles all gender neutral names in each comp_gn_names file to create a long list of all gender neutral names in every decade along with the accompanying year. 
```{r}
#load data files
...
```
An excerpt of the head and tail of the CSV file that was written, which shows all names along with the year that matched is shown here.
```{r}
head(gn_all,10)
tail(gn_all,10)
```