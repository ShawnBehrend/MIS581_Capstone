...
```
Using the All Decades Summary Table created in a previous step, correlation tests were run for the occurrences of gender neutral names versus the total occurrences of any name, for both males and females.
```{r}
# Load ggplot2
library("ggplot2")
...
sink()  # returns output to the console
```
The results of the correlation tests were saved to .txt files above. Those results are shown below.
```{r}
cortest_f
cortest_m
```
